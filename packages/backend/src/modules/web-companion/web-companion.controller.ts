...
import { CreateUploadItemsDtoSchema, type CreateUploadItemsDto } from "./dto/create-upload-items.dto.ts";
import type {
...
export class WebCompanionController {
  ...
  async createUploadItems(
    ...
    try {
      const dto = parseDto<CreateUploadItemsDto>(
      ...
      const files = dto.files
        ?? dto.items?.map((item, index) => ({
          clientFileId: item.clientFileId ?? `${index}-${item.filename}`,
  ...
  async createShareToken(
    sessionId: string,
    request: Omit<CreateShareTokenInput, 'sessionId' | 'sessionToken'>,
    token?: string,
  ...
  async accessSharedContent(
    ...
    clientIp?: string,
    userAgent?: string,
  ): Promise<{ isValid: boolean; shareToken?: any; error?: string }> {
    try {
