...
test("books generation has a single OpenAI Agents SDK production runner", () => {
  ...
  assert.doesNotMatch(runner, /chat\.completions/);
});
...
test("sidecar uses nestjs as the production http runtime", () => {
  ...
  assert.equal(packageJson.scripts.dev, "node src/main.ts");
  ...
  assert.match(packageJson.scripts["check:tests"], /node --test/);
...
test("nestjs services use the registerInjectable helper for strip-only runtime metadata", () => {
  const expected = new Map([
    ["dataset", /registerInjectable\(DatasetService, \[DatasetStateService, AppConfigService\]\)/],
    ["books", /registerInjectable\(BooksService, \[AppConfigService, DatasetStateService, FileJobStoreService, AgentConfigApplicationService\]\)/],
    ["config", /registerInjectable\(ConfigService, \[AppConfigService, PrismaService, PrismaMigrationService\]\)/],
  ]);

  for (const [feature, helperPattern] of expected) {
    ...
    assert.match(service, helperPattern);
    assert.doesNotMatch(service, new RegExp(`Inject\\([^)]*\\)\\(${serviceName}`));
    assert.doesNotMatch(service, new RegExp(`Injectable\\(\\)\\(${serviceName}\\)`));
  }
});
...
test("runtime-loaded sidecar typescript avoids constructor parameter properties", () => {
  const roots = [
  ...
  for (const dir of roots) {
    for (const file of fs.readdirSync(dir)) {
      ...
      if (source.includes("constructor(private readonly")) {
        offenders.push(path.relative(root, filePath));
      }
    }
  }

  assert.deepEqual(offenders, [], `strip-only runtime does not support parameter properties:\n${offenders.join("\n")}`);
});
