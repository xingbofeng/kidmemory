...
export function UploadView({ activeSession, sessionToken, onBack }: UploadViewProps) {
  ...
  return (
    <section className="upload-view" aria-labelledby="upload-title">
      ...
      {activeSession ? <FileUpload session={activeSession} sessionToken={sessionToken} /> : <div className="inline-alert danger">{t('webCompanion.uploadNeedSession')}</div>}
    </section>
  )
}
