...
export function createDatasetService(dependencies: DatasetDependencies) {
  ...
  return {
    ...
    async createChild(input: { id?: string; name?: string; birthday?: string; notes?: string; metadata?: Record<string, unknown> }) {
    ...
    async updateChild(
      id: string,
      updates: { name?: string; birthday?: string; notes?: string; metadata?: Record<string, unknown> },
    ) {
      const db = await dependencies.datasetState.activatePersistent();
      ...
      await db.upsertChild(next);
      return { status: 200, data: { child: next } };
    },
    ...
    async deleteAssetsBatch(ids: string[]) {
      const db = await dependencies.datasetState.activatePersistent();
      let deleted = 0;
      for (const id of ids) {
        const ok = await db.deleteAsset?.(id);
        if (ok === true) deleted += 1;
      }
