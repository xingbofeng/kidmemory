...
async function resolveDefaultSampleDatasetUrl() {
  const candidate = new URL("../../../../examples-dataset/", import.meta.url);
  await fs.access(new URL("child.json", candidate));
  return candidate;
}

export async function listAssets(db: SampleDb, filter: { type?: string; childId?: string; query?: string } = {}) {
  const assets = await db.getAssets(filter);
