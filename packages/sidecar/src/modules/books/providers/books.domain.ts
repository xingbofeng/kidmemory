  ...
  return {
    async createJob(body: Record<string, any>) {
      try {
        ...
        let runner;
        try {
          runner = await dependencies.agentRunner.generateBook(runnerInput, agentConfig);
        } catch (error) {
          const message = error instanceof Error ? error.message : "Unknown runner error";
          runner = { ok: false, message };
        }
        ...
        return { status: 500, data: { ok: false, message } };
      }
    },
    ...
    async listJobs() {
      const jobs = await dependencies.jobStore.list();
      jobs.sort((a, b) => {
    ...
    async cancelJob(id: string) {
      ...
      if (!job) return { status: 404, data: { ok: false, message: "Job not found" } };
      if (job.status === "exported") {
        return { status: 409, data: { ok: false, message: "Exported job cannot be cancelled." } };
      }
      if (job.status === "cancelled") {
        return { status: 200, data: job };
      }
      const cancelled = await dependencies.jobStore.save({ ...job, status: "cancelled" });
      return { status: 200, data: cancelled };
    },
    async retryJob(id: string) {
      const job = await dependencies.jobStore.get(id);
      if (!job) return { status: 404, data: { ok: false, message: "Job not found" } };
      if (!Array.isArray(job.selectedAssetIds) || job.selectedAssetIds.length === 0) {
        return { status: 422, data: { ok: false, message: "Job is missing retry metadata (selectedAssetIds)." } };
      }
      const childId = job.childId || await inferChildIdFromSelectedAssets(dependencies, job.selectedAssetIds);
      if (!childId) {
        return { status: 422, data: { ok: false, message: "Job is missing retry metadata (childId)." } };
      }
      return this.createJob({
        childId,
        assetIds: job.selectedAssetIds,
      });
    },
...
async function inferChildIdFromSelectedAssets(dependencies: BooksDependencies, assetIds: string[]) {
  try {
    const db = await dependencies.datasetState.activatePersistent();
    const assets = await db.getAssetsByIds(assetIds);
    const childIds = [...new Set(assets.map((asset) => asset.childId).filter(Boolean))] as string[];
...
async function resolveDefaultAgentConfig(dependencies: BooksDependencies): Promise<ResolveAgentConfigResult> {
  ...
  try {
    apiKey = await service.getDecryptedApiKey(config.id);
  } catch (error) {
