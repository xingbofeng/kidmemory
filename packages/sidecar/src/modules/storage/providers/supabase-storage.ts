  ...
  return {
    ...
    async createSignedUploadUrl(objectPath: string) {
      ...
      return {
        ...
        action: "Configure Supabase S3 credentials or fix the configured service role key.",
        retryable: false,
        url: "",
        expiresInSeconds: config.signedUrlTtlSeconds,
      };
...
function validateConfig(config: SupabaseStorageConfig): StorageActionResult {
  ...
  if (config.s3.accessKeyId.trim() || config.s3.secretAccessKey.trim()) {
    return {
      ...
      action: "Configure the Supabase S3 endpoint, bucket, access key, and secret key.",
      retryable: false,
    };
  }
  if (!config.url.trim() || !config.bucket.trim() || !config.serviceRoleKey.trim()) {
    return {
      ...
      action: "Configure the Supabase URL, Storage bucket, and service role key.",
      retryable: false,
    };
  }
  return { ok: true };
...
function mapStorageResponseError(status: number, body: string): StorageActionResult {
  ...
  if (status === 404) {
    return {
      ...
      action: "Check the configured Storage bucket and the target object path.",
      retryable: false,
    };
  }
