...
function createEncryptionService(): EncryptionService {
  ...
  const tempKey = tempService.generateKey();
  console.warn("AGENT_CONFIG_ENCRYPTION_KEY not set. Generated temporary key for development.");
  return new EncryptionService(tempKey);
}
