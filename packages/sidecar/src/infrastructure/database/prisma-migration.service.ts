...
export class PrismaMigrationService {
  ...
  constructor(config: AppConfigService, runner: ProcessRunner = spawnProcess, cwd = path.resolve(import.meta.dirname, "../../..")) {
    ...
    this.runner = runner;
    this.cwd = cwd;
    this.prismaCliPath = path.join(this.cwd, "node_modules", "prisma", "build", "index.js");
  }

  async deploy(): Promise<PrismaMigrationResult> {
    return this.runPrismaCommand(["migrate", "deploy"], "Prisma migrations applied successfully.");
  }
  ...
  private async runPrismaCommand(args: string[], successMessage: string): Promise<PrismaMigrationResult> {
    ...
    const commandArgs = [this.prismaCliPath, ...args];
    const env = {
      ...prismaProcessEnv(process.env),
      DATABASE_URL: postgresConnectionUrl(this.config.config),
    };
    const result = await this.runner(command, commandArgs, { cwd: this.cwd, env });
    ...
    return {
      ...
      service: "prisma-migrate",
      command: commandLabel,
      message: buildFailureMessage(result.code, result.stderr, result.stdout),
      stdout: sanitizeMigrationOutput(result.stdout),
      stderr: sanitizeMigrationOutput(result.stderr),
    };
  }
...
function prismaProcessEnv(source: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  const env = { ...source };
  for (const key of Object.keys(env)) {
    if (
      key.startsWith("SWIFT_") ||
      key.startsWith("LLVM_") ||
      key.startsWith("CLANG_") ||
      key.startsWith("GCC_")
    ) {
      delete env[key];
    }
...
function buildFailureMessage(
  ...
  const normalizedStderr = sanitizeMigrationOutput(stderr).trim();
  const normalizedStdout = sanitizeMigrationOutput(stdout).trim();
  const isNoiseLine = (line: string) => {
    const text = line.trim();
    if (!text) return true;
    return (
      /^warning:\s*unknown environment variable\b/i.test(text) ||
      /^loaded prisma config from\b/i.test(text) ||
      /^prisma schema loaded from\b/i.test(text) ||
      /^start by importing your prisma client\b/i.test(text)
    );
  };
  const pickDetail = (value: string) =>
    value
      .split(/\r?\n/)
      .map((line) => line.trim())
      .find((line) => !isNoiseLine(line));
  const detail =
    pickDetail(normalizedStderr) ?? pickDetail(normalizedStdout);
  if (detail) {
    return `Prisma migration failed with exit code ${code ?? "unknown"}: ${detail}`;
  }
  return `Prisma migration failed with exit code ${code ?? "unknown"}.`;
}
