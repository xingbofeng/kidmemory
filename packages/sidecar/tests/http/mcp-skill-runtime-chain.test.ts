...
test("MCP tools expose skill runtime chain and enforce boundaries", async (t) => {
  ...
  const skillRunResult = await waitForSkillRuntimeReady(client);
...
async function waitForSkillRuntimeReady(client: Client) {
  const startedAt = Date.now();
  let lastResult: Record<string, unknown> = {};

  while (Date.now() - startedAt < 15000) {
    lastResult = decodeJsonResult(await client.callTool({
      name: "run_skill_task",
      arguments: {
  ...
  throw new Error(`skill runtime did not become ready in time: ${JSON.stringify(lastResult)}`);
}
