...
function createDirectUploadConfig(overrides: {
  supabaseUrl?: string;
  ...
  config.supabaseStorage.url = overrides.supabaseUrl ?? "https://kidmemory.supabase.co";
  config.supabaseStorage.serviceRoleKey = overrides.supabaseServiceRoleKey ?? "supabase-service-role-secret";
  config.supabaseStorage.anonKey =
    overrides.supabaseAnonKey ?? "supabase-anon-public-but-routed-through-sidecar";
  config.webCompanionDirectUpload.bucket = overrides.bucket ?? "web-companion-uploads";
  config.webCompanionDirectUpload.publicUrl = overrides.publicUrl ?? "https://kidmemory-companion.example.com";
  config.webCompanionDirectUpload.enabled = overrides.enabled ?? true;
...
test(
  ...
  () => {
    const config = createDirectUploadConfig({ supabaseUrl: "" });
...
test(
  ...
  () => {
    const config = createDirectUploadConfig({ supabaseAnonKey: "" });
...
test(
  ...
  () => {
    const config = createDirectUploadConfig({ publicUrl: "" });
...
test(
  ...
  () => {
    const config = createDirectUploadConfig({ supabaseServiceRoleKey: "" });
...
test(
  ...
  () => {
    const config = createDirectUploadConfig({ enabled: false });
...
test(
  ...
  () => {
    for (const value of ["1", "true", "yes", "on", "TRUE"]) {
      const config = loadConfigFromEnv({ WEB_COMPANION_DIRECT_UPLOAD_ENABLED: value });
      assert.equal(
