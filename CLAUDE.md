...
```bash
cd packages/backend && npm install              # 安装依赖（需要 Node 22+）
cd packages/backend && npm run dev              # 启动开发服务器（src/main.ts）
cd packages/backend && npm test                 # 运行所有测试：find tests -name '*.test.ts' | xargs node --test
cd packages/backend && npm run check:src        # 检查运行时导入
cd packages/backend && npm run check:tests      # 检查测试导入
cd packages/backend && npm run build            # 完整构建检查

# 运行单个测试文件：
cd packages/backend && node --test tests/unit/asset-import.test.ts

# 使用 Supabase 本地数据库运行测试（推荐）：
cd packages/backend && supabase start           # 启动本地 PostgreSQL + pgvector
cd packages/backend && npm test                 # 运行测试
cd packages/backend && supabase stop            # 停止本地数据库
```
...
- `packages/backend/` — NestJS HTTP API + 任务编排（TypeScript，Node.js 运行时）
...
2. 启动 sidecar：`cd packages/backend && npm run dev`
3. 启动 Flutter 桌面端：`cd packages/desktop && flutter run -d macos`
