...
# cloud-api 单元测试
cd packages/cloud-api && npm run test:unit
...
详细测试与验收门禁请查看 [docs/deployment/ci-cd.md](docs/deployment/ci-cd.md)
...
│   ├── sidecar/          # 本地 NestJS sidecar（桌面本地能力）
│   ├── cloud-api/        # 云端协作 API（上传/分享/同步）
│   ├── protocol/         # 统一协议层 + OpenAPI/类型生成
│   └── web/              # Web 伴侣（手机端）
├── docs/                 # 产品文档和设计资源
├── templates/            # 书籍模板
├── examples/             # 示例数据集
...
# 架构测试
cd packages/sidecar && node --test tests/architecture/architecture.test.ts
```
...
cd kidmemory/packages/cloud-api
cp .env.example .env
# 编辑 .env
```
...
- `TENCENT_HOST`：服务器 IP
- `TENCENT_USER`：服务器登录用户
- `TENCENT_SSH_KEY`：部署私钥
- `TENCENT_PROJECT_PATH`：服务器项目路径（例如 `/home/ubuntu/kidmemory`）
- `TENCENT_SMOKE_BASE_URL`：可选，部署后健康检查地址

3. 首次手动部署

```bash
cd /home/<user>/kidmemory/packages/cloud-api
npm install
npm run build
...
### 用户文档
- [文档总览](docs/README.md) - 文档入口
- [架构总览](docs/architecture/service-split.md) - sidecar / cloud-api 分层说明

### 开发文档
- [开发指南](CLAUDE.md) - Claude Code 工作指导
- [API 文档](docs/api/README.md) - sidecar/cloud-api 接口说明
- [架构文档](docs/product/architecture.md) - 技术架构详解
- [CI/CD](docs/deployment/ci-cd.md) - 验收与流水线

### 发布文档
- [腾讯云部署](docs/deployment/tencent-cloud.md)
- [Vercel 部署](docs/deployment/vercel.md)
